...
interface AdminPack { id: string; title: string; soundCount: number; authorName: string; downloads: number; }
...
const AdminPanel: React.FC<Props> = ({ isOpen, onClose, onRefresh }) => {
  const [tab, setTab] = useState<'sounds' | 'pending' | 'packs' | 'users' | 'reports' | 'broadcast'>('sounds');
  ...
  const load = useCallback(async () => {
    const [s, p, pk, u, r] = await Promise.all([
      aApi('/admin/sounds'), aApi('/admin/pending'), aApi('/packs'),
      aApi('/admin/users'), aApi('/admin/reports'),
    ]);
    if (Array.isArray(s)) setSounds(s);
    if (Array.isArray(p)) setPending(p);
    if (Array.isArray(pk)) setPacks(pk);
  ...
  const delPack = async (id: string) => { await aApi('/admin/packs/delete', { packId: id }); setPacks(p => p.filter(pk => pk.id !== id)); onRefresh(); };
  const delUser = async (id: string) => {
    if (!confirm('Удалить аккаунт? Звуки и биты останутся на сайте.')) return;
    const r = await aApi('/admin/users/delete', { userId: id });
    if (r?.ok) { setUsers(p => p.filter(u => u.id !== id)); onRefresh(); }
  };
  ...
  const fPk = useMemo(() => !q ? packs : packs.filter(p => p.title.toLowerCase().includes(q) || p.authorName.toLowerCase().includes(q)), [packs, q]);
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6">
        ...
        <div className="flex gap-4 mb-5 border-b border-[#EBEBEB] overflow-x-auto">
          {([
            ...
            { id: 'packs' as const, label: `Паки (${fPk.length})` },
        ...
        {tab === 'packs' && (
          <div className="space-y-1.5">
            {fPk.length === 0 ? <p className="text-[13px] text-[#999] py-8 text-center">Нет паков</p> : fPk.map(p => (
              <div key={p.id} className="flex items-center gap-2 sm:gap-3 bg-white border border-[#EBEBEB] rounded-xl px-3 sm:px-4 py-2.5">
                <div className="flex-1 min-w-0">
                  <div className="text-[13px] font-medium text-[#0A0A0A] truncate">{p.title}</div>
                  <div className="text-[10px] text-[#999] truncate">{p.authorName} · {p.soundCount} звуков · {p.downloads} скач.</div>
                </div>
                <button onClick={() => delPack(p.id)} className="px-2 sm:px-3 py-1 text-[10px] font-semibold text-red-500 border border-red-200 rounded-lg hover:bg-red-50 transition-all shrink-0">Удалить</button>
              </div>
            ))}
          </div>
        )}
            ...
            {fU.map(u => (
              <div key={u.id} className="flex items-center gap-2 sm:gap-3 bg-white border border-[#EBEBEB] rounded-xl px-3 sm:px-4 py-2.5">
                ...
                <div className="flex gap-1 shrink-0">
                  ...
                  {u.email !== ADMIN_EMAIL && <button onClick={() => delUser(u.id)} className="px-2 py-1 text-[10px] font-semibold text-red-500 border border-red-200 rounded-lg hover:bg-red-50 transition-all">Удалить</button>}
                </div>
              </div>
            ))}
          </div>
              ...
              <div key={r.id} className="bg-white border border-[#EBEBEB] rounded-xl p-4">
                <div className="flex items-start justify-between mb-2">
                  <div>
                    ...
                    <div className="text-[10px] text-[#999]">{r.user_email} · {new Date(r.created_at).toLocaleString('ru')}</div>
                  </div>
